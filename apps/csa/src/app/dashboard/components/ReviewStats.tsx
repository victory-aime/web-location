...
import { Badge, FormatNumber, HStack, Stat, Flex, useBreakpointValue } from '@chakra-ui/react'
import { BoxContainer } from '_components/custom'
import { hexToRGB, Colors } from '_theme/colors'
...
export const ReviewStats: FC<StatsProps> = ({
  ...
  return (
    ...
    >
      <Flex borderRadius={'full'} p={'14px'} alignItems={'center'} justifyContent={'center'} bgColor={hexToRGB(color, 0.2, opacity)} color={iconColor}>
        {icon && icon}
      </Flex>
      ...
      <Stat.Root {...rest}>
        ...
        <HStack width={'full'} alignItems={'center'} justifyContent={'space-between'}>
          <Stat.ValueText alignItems={'center'} justifyContent={'center'}>
            <FormatNumber notation="compact" style={'currency'} currency={'USD'} value={value} />
          </Stat.ValueText>
          <Badge colorPalette={badgeColor} gap="4px" p={'8px'} borderRadius={'full'}>
            {badgeColor !== 'red' ? <Stat.UpIndicator color={badgeColor} /> : <Stat.DownIndicator />}
