...
import { Chart as ChartJS, Tooltip, Legend, LinearScale, CategoryScale, BarElement, Title, Plugin, ChartData, ChartOptions } from 'chart.js'
  ...
  const options: ChartOptions<'bar'> = {
    ...
    scales: {
      ...
      y: {
        display: false,
        ticks: {
          stepSize: Math.ceil(Math.max(...Object.keys(formatData.maxValues ?? {}).map((key) => formatData.maxValues?.[key] ?? 0)) / 5),
        },
        stacked: true,
        border: {
          display: false,
      ...
      {!dataChart || loader ? (
        ...
        <Box>
          <Box width={'100%'} minHeight={'300px'} mt={'30px'} mb={'30px'}>
            {Math.max(...Object?.values(formatData?.maxValues || {})) < 0 ? null : <Bar ref={chartRef} data={data} options={options} plugins={[hoverSegment]} />}
          </Box>
          <Center mt={'20px'}>
