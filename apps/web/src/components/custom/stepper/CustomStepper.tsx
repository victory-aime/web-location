import { StepsContent, StepsItem, StepsList, StepsNextTrigger, StepsPrevTrigger, StepsRoot } from '_components/ui/steps'
import { Box, For, useBreakpointValue, VStack } from '@chakra-ui/react'
import React, { FC, useEffect, useState } from 'react'
import { StepperProps } from './interface/stepper'
import { BaseText, TextVariant } from '../base-text'

export const CustomStepper: FC<StepperProps> = ({ steps, goNextSteps, stepTitle }) => {
            ...
            <Box width={'full'} key={index}>
              {responsive ? (
                <VStack gap={2} key={index} width={'full'} alignItems={'start'}>
                  <StepsItem width={'full'} key={index} index={index} icon={item?.icon} />
                  <BaseText variant={TextVariant.XS}>{item?.label}</BaseText>
                </VStack>
              ) : (
                <StepsItem key={index} width={'full'} title={item?.label} index={index} icon={item?.icon} />
              )}
            </Box>
          )}
        </For>
      ...
      <Box mt={5}>
        <For each={steps}>
          {(item, index) => (
            ...
            >
              {item.content({
                NextTrigger: ({ children }) => <StepsNextTrigger>{children}</StepsNextTrigger>,
                PrevTrigger: ({ children }) => <StepsPrevTrigger>{children}</StepsPrevTrigger>,
              })}
            </StepsContent>
          )}
        </For>
