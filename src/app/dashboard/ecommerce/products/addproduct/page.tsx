...
import { FormTextInput, FormTextArea, FormSelect } from '_components/custom/form';
...
import { ProductModule, UsersModule } from '_store/src/modules';
...
const AddProductPage = () => {
  ...
  const { addProduct, products, isLoading, categories, updateProduct } = useSelector(
  ...
  const existingProductFiles = UTILS.findDynamicIdInList(requestId ?? '', products);
                  ...
                  >
                    <Box mt={5} width="full">
                      <Text color={'gray.400'} mb={5}>
                        Les variations vous permettent d'ajouter différentes options pour un même
                        produit, comme la couleur ou la taille. Ajoutez celles qui s'appliquent à
                        votre article
                      </Text>
                      {values?.variants &&
