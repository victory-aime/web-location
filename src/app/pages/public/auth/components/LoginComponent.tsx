...
const LoginComponent = () => {
  ...
  const { isLoggedIn, isLoading, currentUser } = useSelector(AuthModule.selectors.authSelector);
  ...
  return (
    <Box mt={'30px'}>
      <Center alignItems={'flex-start'} justifyContent={'flex-start'} flexDir={'column'} gap={4}>
        <BaseText variant={TextVariant.L}>Bon retour parmi Nous!</BaseText>
        <BaseText color={'gray.500'}>Veuillez saisir vos identifiants de connexion</BaseText>
        ...
        <Flex width={'full'} gap={2} alignItems={'center'} justifyContent={'center'}>
          ...
          <Link href={APP_ROUTES.CLIENT_PAGES.PUBLIC.SIGN_UP}>
            <BaseText variant={TextVariant.S} color={'blue.500'} weight={TextWeight.Medium}>
              Connectez-vous
            </BaseText>
          </Link>
        </Flex>
